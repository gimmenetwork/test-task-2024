...
<template>
  ...
  <div class="flex flex-col items-center justify-center px-6 py-2 mx-auto lg:py-0">
      <div class="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0 bg-gradient-to-r from-bermuda-gray-100 to-bermuda-gray-200">
          <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form class="space-y-4 md:space-y-6" @submit="onSubmit">
                  <div>
                      ...
                      <label for="title" class="block mb-2 text-sm font-medium text-gray-900 text-black">Book Title</label>
                      <input type="text" v-bind="titleAttrs" name="title" aria-errormessage="err-title" v-model="title" class="bg-gray-50 text-black sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-3" placeholder="Enter a title for your book">
                  </div>
                  <div>
                      <p id="err-pageCount" class="text-red-500">{{ errors.pageCount }}</p>
                      <label for="pageCount" class="block mb-2 text-black text-sm font-medium text-gray-900">Book Page Count</label>
                      <input type="number" min="1" v-bind="pageCountAttrs" name="pageCount" aria-errormessage="err-pageCount" v-model="pageCount" placeholder="Enter book pages count" class="bg-gray-50 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-3">
                  </div>
                  <div>
                    ...
                    <select v-model="genre" v-bind="genreAttrs" name="genre" id="genres" aria-errormessage="err-pageCount" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
