...
<template>
    <section>
        <div v-if="props.pageCount === props.pagesRead" class="pt-2">
            ...
            <div v-else>
                <button class="w-30 bg-bermuda-gray-300 px-10 py-2 rounded-xl " @click="toggleReview"
                    v-if="!showReview">Leave a Review</button>
                <button
                    class="w-30 bg-red-400 px-3 py-1 rounded-xl text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium me-2 mb-2 focus:outline-none"
                    @click="toggleReview" v-if="showReview">X</button>
            </div>
        </div>
        <div v-if="showReview">
            <div>
                <form @submit="onSubmit">
                    ...
                    <textarea v-bind="reviewAttrs" name="review" aria-errormessage="err-review" v-model="review" id="review"
                        rows="5"
