...
<template>
    <div class="flex flex-row justify-between py-2">
                    ...
                    class="w-30 text-sm bg-bermuda-gray-500 px-3 py-1 rounded-xl text-white bg-blue-700 hover:bg-bermuda-gray-600 focus:ring-4 focus:ring-blue-300 font-medium me-2 mb-2 focus:outline-none">Add new progress</button>
                    <button class="w-30 px-3 rounded-xl text-white bg-red-300 hover:bg-red-500 focus:ring-4 focus:ring-blue-300 font-medium me-2 mb-2 focus:outline-none" @click="toggleProgress" v-if="showAddProgress">X</button>
            </div>
    <div v-if="showAddProgress" class="pb-4">
                <form @submit="onSubmit">
                    ...
                    <input type="number" min="1" :max="pageCount" v-bind="pageReadAttrs" name="pageRead" aria-errormessage="err-pageRead" v-model="pageRead" id="pageRead" rows="5" class="block p-2.5 w-full mb-3 text-sm text-gray-900 bg-gray-50 rounded-lg border border-bermuda-gray-400 focus:ring-blue-500 focus:border-blue-500" placeholder="Enter your progress" />
