...
<template>
        ...
        <section v-for="book in Books" :key="book.id" class="max-w-sm font-semibold bg-white  border-gray-200 rounded-lg shadow py-4 px-5 my-4 mx-auto">
            <section class="p-5">
                ...
                <p class="text-md text-bermuda-gray-600">{{ parseInt(book.pagesRead) }} pages of {{ parseInt(book.pageCount) }} read</p>
                <div>
                ...
                <div>
                    <p class="text-sm text-green-500">You have only {{ parseInt(book.pageCount) - parseInt(book.pagesRead) }} pages left to read</p>
                </div>
